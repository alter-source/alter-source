...
class CErrorLaserDot;
class RocketTrail;
 
//###########################################################################
//	>> CErrorMissile		(missile launcher class is below this one!)
//###########################################################################
class CErrorMissile : public CBaseCombatCharacter
{
	DECLARE_CLASS( CErrorMissile, CBaseCombatCharacter );
...
public:
	...
	CHandle<CWeaponGrenadeLauncher>		m_hOwner;

	static CErrorMissile *Create( const Vector &vecOrigin, const QAngle &vecAngles, edict_t *pentOwner );

protected:
	virtual void DoExplosion();	
	virtual void ComputeActualDotPosition( CErrorLaserDot *pErrorLaserDot, Vector *pActualDotPosition, float *pHomingSpeed );
	...
	void GetShootPosition( CErrorLaserDot *pErrorLaserDot, Vector *pShootPosition );
...
//-----------------------------------------------------------------------------
class CAPCErrorMissile : public CErrorMissile
{
	DECLARE_CLASS( CErrorMissile, CErrorMissile );
	DECLARE_DATADESC();

public:
	static CAPCErrorMissile *Create( const Vector &vecOrigin, const QAngle &vecAngles, const Vector &vecVelocity, CBaseEntity *pOwner );
...
private:
	...
	void ExplodeThink();
	void APCErrorMissileTouch( CBaseEntity *pOther );
...
class CWeaponGrenadeLauncher : public CBaseHL2MPCombatWeapon
{
	DECLARE_CLASS( CWeaponGrenadeLauncher, CBaseHL2MPCombatWeapon );
public:
...
#ifndef CLIENT_DLL
	CHandle<CErrorLaserDot>	m_hLaserDot;
#endif
