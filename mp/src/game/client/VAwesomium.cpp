...
void VAwesomium::OpenHTMLFile(const char *filePath)
{
	if (!filePath)
	{
		Warning("[HTML-ERR] Invalid file path.\n");
		return;
	}

	FileHandle_t fileHandle = filesystem->Open(filePath, "rb", "MOD");
	if (!fileHandle)
	{
		Warning("[HTML-ERR] Failed to open file: %s\n", filePath);
		return;
	}
	...
	if (fileSize <= 0)
	{
		Warning("[HTML-ERR] File is empty or could not get size: %s\n", filePath);
	...
	int bytesRead = filesystem->Read(fileContent, fileSize, fileHandle);
	if (bytesRead != fileSize)
	{
		Warning("[HTML-ERR] Error reading file: %s\n", filePath);
		filesystem->Close(fileHandle);
		delete[] fileContent;
		return;
	}
	...
	g_pFullFileSystem->RelativePathToFullPath(filePath, "MOD", fullPath, sizeof(fullPath));

	char url[MAX_PATH * 2];
	Q_snprintf(url, sizeof(url), "file:///%s", fullPath);
